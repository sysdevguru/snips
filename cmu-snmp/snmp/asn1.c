...
static char rcsid[] =
"$Id: asn1.c,v 1.9 1998/05/06 04:06:47 ryan Exp $";
  ...
  /*    int     intsize;       IN     - size of output buffer */
{
  ...
  *type = *bufp++;
  ...
  /* Can we store this int? */
  if ((asn_length > ((u_int)intsize + 1)) ||
      ((asn_length == (u_int)intsize + 1) && *bufp != 0x00)) {
    snmp_set_api_error(SNMPERR_ASN_DECODE);
    return(NULL);
  }
  ...
  /*    u_char  *type;       OUT - ASN type of object */
{
  ...
  *type = *bufp;
  ...
  if (header_len + asn_length > (u_int)(*datalength)) {
    snmp_set_api_error(SNMPERR_ASN_DECODE);
    return(NULL);
  }
 ...
 *  Returns NULL on any error.
 */
static u_char *asn_build_sequence(u_char *data, int *datalength, 
