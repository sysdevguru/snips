...
/* THIS IS THE MODIFIED SNMPTRAPD.C v1.12 FOR SNIPS */

/*
 * SNMP trap monitor writes events to SNIPS data file. Expires events
 * from the datafile after N minutes (since there is no way to delete
 * trap events.
 *
 * AUTHOR:
...
#include <snmp/snmp.h>
/* #include "version.h"		/* has following code */
char *snmpapps_Version(void);

/* #include "options.h"	*/		/* defines TRAPD_LOG_ENTERPRISE */
#define TRAPD_LOG_ENTERPRISE 1

static char rcsid[] = 
"$Id: trapmon.c,v 1.1 2008/04/25 23:31:53 tvroon Exp $";
...
#ifdef SNIPS
    if (Snips) {
      char *ipaddr = inet_ntoa(pdu->address.sin_addr);
      char *hname;
      struct hostent *h;

      if ((h = gethostbyaddr((char *)&(pdu->address.sin_addr),
			     sizeof(unsigned long), AF_INET)) == NULL)
	hname = ipaddr;
      else
	hname = h->h_name;
 
      add_snips_event(ipaddr, hname, pdu);
    }	/* if (Snips) */
#endif
...
void Usage(void)
{
  fprintf(stderr, "Usage: trapmon [-P] [-V] [-S] [-d] [-p port]\n");
  fprintf(stderr, "\n");
...
#ifdef SNIPS
  fprintf(stderr, "  -s     SNIPS mode (writes snips datafile)\n");
#endif
  exit(1);
}
...
#ifdef SNIPS
  if (argc <= 1)
    ++Snips;	/* no options, so assume Snips mode */
  if (Snips) init_snips();
#endif
